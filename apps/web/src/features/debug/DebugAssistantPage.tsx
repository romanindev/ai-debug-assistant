...
export function DebugAssistantPage() {
  ...
  const displayedAnalysis = analyzeMutation.data ?? lastSuccessfulAnalysis?.result;
  const displayedContext = analyzeMutation.data
    ? context
    : lastSuccessfulAnalysis?.request.context;
  const canRetry = lastRequest !== null && !analyzeMutation.isPending;
  const showEmptyState =
    !displayedAnalysis && !analyzeMutation.isPending && !analyzeMutation.isError;
  ...
  function handleRetry() {
    if (!lastRequest || analyzeMutation.isPending) {
      return;
    }
  ...
  function handleCopyFullAnalysis() {
      ...
      <section className="grid grid-cols-[minmax(340px,0.9fr)_minmax(420px,1.1fr)] items-start gap-5 max-[860px]:grid-cols-1">
        ...
        >
          <div className="mb-4 flex items-center justify-between gap-4">
            ...
            {displayedContext && (
              <span className="text-[13px] text-slate-500">
                {contextLabels[displayedContext]}
              </span>
            )}
          </div>
          ...
          {analyzeMutation.isPending && (
            <div
              className={
                displayedAnalysis
                  ? 'mb-4'
                  : 'grid min-h-[500px] place-content-center text-center'
              }
            >
          ...
          {analyzeMutation.isError && (
            ...
            >
              ...
              <button
                className="mt-4 min-h-10 rounded-lg border border-red-300 px-4 font-bold text-red-800 disabled:cursor-not-allowed disabled:opacity-60"
                type="button"
                disabled={!canRetry}
                onClick={handleRetry}
              >
          ...
          {displayedAnalysis && (
            <div className="grid gap-3.5">
              <div className="flex items-center justify-end gap-2">
                ...
                <button
                  className="min-h-9 rounded-lg border border-slate-300 px-3 text-[13px] font-bold text-slate-700"
                  type="button"
              ...
              <ResultBlock title="Summary" content={displayedAnalysis.summary} />
              <ResultBlock
                title="Possible Cause"
                content={displayedAnalysis.possibleCause}
              />
              <ResultBlock
                title="Suggested Fix"
                content={displayedAnalysis.suggestedFix}
              />

              {displayedAnalysis.codeExample && (
                <section className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        void handleCopy(displayedAnalysis.codeExample ?? '', 'Code')
                      }
                    >
                      Copy code
                    </button>
                  ...
                  <pre className="m-0 overflow-x-auto rounded-lg bg-slate-950 p-3 text-[13px] leading-6 text-blue-100">
                    <code>{displayedAnalysis.codeExample}</code>
                  </pre>
                </section>
              )}
              ...
              <section className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                ...
                <ul className="m-0 list-disc pl-4 text-sm leading-6 text-slate-700">
                  {displayedAnalysis.checklist.map((item) => (
...
function formatAnalysisForCopy(analysis: DebugAnalysis): string {
  return [
    `Summary\n${analysis.summary}`,
    `Possible Cause\n${analysis.possibleCause}`,
    `Suggested Fix\n${analysis.suggestedFix}`,
    analysis.codeExample ? `Code Example\n${analysis.codeExample}` : null,
